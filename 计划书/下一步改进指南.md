...
## 状态核销表（v10.5.0 已交付，2026-09-18 实测）

> 本章对应批次已在 v10.5.0 真实落地（提交 `0617ae0`/`be28c3e`/`da7f89b`/`eb15f81`，
> 加上 CI 修复 `3509dad`）。执行者不要再做，直接跳到未核销批次。

| 批次 | 内容 | 证据（本会话实测） |
|------|------|------|
| P0-A | v10.4.0 基线按主题提交 + 版本五同步 + push + tag + Release | 10 个主题 commit（`fc1625a`..`04c9a49`）；tag `v10.4.0` 已推送；Release 已创建（gh release view 确认 publishedAt）；远程 SHA 04c9a49 一致 |
| P0-B | 启动脚本加固：精确杀进程 + 版本去硬编码 + --dry-run | `cleanup-stale.js` 重写；`desktop/test-cleanup-stale.js` 单测通过（3 命中/3 排除/2 边界）；`test_desktop_wiring.py` 15 passed；`node cleanup-stale.js --dry-run` 实测无误杀 |
| P0-C | check_wiring 快速模式说明 + CI 根因修复 | `check_wiring --check wiring,version,docs` 全过（85~128s，wiring 0 新增）；**CI 历史全红根因**（缺 requirements-web.txt → ModuleNotFoundError: fastapi）已修并推送 |
| P1-10 | step_timeout_sec 软超时接线 | `tests/test_step_timeout.py` 5 passed；回归 56 passed（framework+supervisor+react router） |
| P1-7 | react 实时事件流（hooks→SSE + 打字机 + 工具状态） | `tests/test_agent_realtime_stream.py` 5 passed（实时事件/双 session 隔离/审批队列路由）；E2E agent-realtime 3 passed；前端 tsc 0 错 + next build 17 路由成功 |
| P1-8 | 审批倒计时/快捷键/映射守护/pending 过滤 | 同上（倒计时归零自动拒绝、Enter/Esc、approvalMaps 守护测试）；E2E 弹窗页面正常 |
| P1-9 | 会话管理 UI（新建/切换/删除/续接） | E2E：run_stream 落库 → 刷新后历史下拉出现该会话 |

| v10.5.1 | **CI 历史全红终结 + 跨平台安全/稳定性**（10 项根因修复，见 CHANGELOG [10.5.1]） | **CI 矩阵 6/6 全绿**（macos/ubuntu/windows × 3.10/3.11，1478+ passed/平台，覆盖率 ≥77%，项目历史首次全绿）；sanitize 跨平台逃逸修复、batch 校验顺序修复、_port_available 裸 bind 诚实探测 |

| v10.5.2 | CI E2E 修复（Node 20 + 端口对齐） | **v10.5.2 tag 全链路 CI = SUCCESS**：矩阵 6/6（macos/ubuntu/windows × 3.10/3.11）+ e2e（真实 Playwright + 真实后端 :8002）+ build-windows（全量测试 + pyinstaller 构建）——发布级完整验证首次全绿 |

| v10.6.0 | P1-1 术语气泡（词表+组件+页面接入+守护测试）+ P1-6 前端可靠性（api 统一超时/GET 重试 + 全局 Toast） | `tsc --noEmit` 0 错；`next build` 17 路由；`tests/test_glossary_toast.py` 4 passed；tag v10.6.0 全链路 CI（矩阵+e2e+build-windows） |

| v10.6.1/10.6.2/10.6.3 | E2E/测试断言容错链（Toast 空态 toBeAttached、gc_guard join 60s、tkinter 真实探测跳过）——纯测试容错，无功能变更 | **v10.6.3 tag 全链路 CI = SUCCESS**：矩阵 6/6 + 真实 E2E + Windows 构建（确定性绿态，覆盖 P1-1/P1-6 新前端） |

**仍未核销（后续迭代主战场）**：P1-1 术语气泡、P1-2 Onboarding、P1-3 用户画像、P1-4 skill 采纳闭环、
P1-5 记忆读侧闭环、P1-6 全局交互层、P2-1~P2-9、P3-1~P3-7、第六章扩展清单（见对应章节）。
