"""Telegram adapter Mock 实现。

真实 TelegramAdapter 未来接入 Bot API（https://api.telegram.org/bot<token>/getUpdates）：
  - getUpdates 长轮询取入站消息
  - sendMessage 发出站回复
  - 凭据（bot_token）从 GatewayCipher 解密后用

本 Mock 不真实连 Telegram，记到内存队列。真实凭据预算 0，标"真实凭据待验证"。
"""
...
class TelegramAdapter(IMAdapter):
    ...
    async def start(self) -> None:
        self._started = True
        logger.info("TelegramAdapter(Mock) started")

    async def stop(self) -> None:
        self._started = False
        logger.info("TelegramAdapter(Mock) stopped")

    async def send(self, peer: str, text: str) -> bool:
        if not self._started:
            logger.warning("TelegramAdapter.send 时 adapter 未 start")
        self.sent_log.append(InboundEnvelope(peer=peer, content=text, raw={}))
        logger.info("TelegramAdapter(Mock) send -> %s: %s", peer, text[:50])
        return True
